...
def hazards_partner_finder(df_hazards_pledge_plan_vertical,p_short_name):
    ...
    st.markdown("Partners are asked to indicate the climate hazards they are addressing when they report their Pledge Statement and when submitting a Plan. The Pledge Statement and Plan Submissions are different sources of information. Please select the one you want to explore:")
    
    # Display the resulting DataFrame
    # df_hazards_pledge_plan_vertical

    # Filter for 'Pledge_n_Plan' only
    # st.write(df_hazards_pledge_plan_vertical)
    
    
    df_hazards_pledge_plan_vertical['source'] = df_hazards_pledge_plan_vertical['source'].replace('Pledge_n_Plan', 'All Tools Available')

    # Create a list of unique options from the DataFrame column
    options = list(df_hazards_pledge_plan_vertical['source'].unique())

    # Create a select box widget
    selected_option = st.selectbox(
        "Select an source of information",
        options)

    
    df_filtered = df_hazards_pledge_plan_vertical[df_hazards_pledge_plan_vertical['source'] == selected_option]
    ...
    plt_all_hazards_pledge_plan = plt.figure(figsize=(10, 6))
    ax = hazard_counts.sort_index().plot(kind='barh', color=colors)

    plt.xlabel(p_short_name)
    plt.ylabel('Hazards')
    plt.title(f'{p_short_name} Hazard ({selected_option})')
    plt.xlim(0, 5)  # Set x-axis limits to show percentages up to 100%

    # # Adding text labels to the bars
    # for index, value in enumerate(hazard_counts.sort_index()):
    #     label = f"{value:.1f} ({hazard_counts.reindex(hazard_counts.sort_index().index).iloc[index]} partner/s)"
    ...
    # st.write('filtered')
    # df_filtered
    st.markdown(f"The bar chart below displays whether hazards are being taken into consideration by {p_short_name}.")
    st.pyplot(plt_all_hazards_pledge_plan)
    ...
    # # Define the number of colors and color range for the bar chart
    # n_colors = len(hazard_group_counts)
    # start = 0.2  # Start from a higher value than 0 to avoid the lightest colors
    # end = 1
    # colors = cm.RdPu(np.linspace(start, end, n_colors))

    # # Create a horizontal bar chart
    # plt_hazards_group_pledge_plan = plt.figure(figsize=(10, 6))
    # sorted_percentages = hazard_group_counts.sort_index()
    # sorted_counts = hazard_group_counts.reindex(sorted_percentages.index)  # Reindex to match the sorted percentages

    # ax = sorted_percentages.plot(kind='barh', color=colors)

    # plt.xlabel('Percentage of Partners (%)')
    # plt.ylabel('Hazard Groups')
    # plt.title(f'Percentage of Partners by Hazard Group {selected_option}')
    ...
    # In Streamlit, you would display this plot using:
    # st.pyplot(plt_hazards_group_pledge_plan)
    ...
    with st.expander("Explore the data on climate hazards"): 
